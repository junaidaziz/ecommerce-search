...
const ProductCard: React.FC<ProductCardProps> = ({
  ...
  return (
    <div
      className={`group relative flex flex-col h-full bg-white dark:bg-zinc-800 border border-gray-100 dark:border-gray-700 rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-shadow duration-300 p-4 gap-4 ${className}`}
    >
      {/* Wishlist Button */}
      <button
        type="button"
        className="absolute top-3 right-3 z-20 p-2 rounded-full bg-white/90 border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-transform duration-200 hover:scale-110 hover:bg-rose-100/70 focus:outline-none focus:ring-2 focus:ring-primary"
        onClick={handleWishlistToggle}
        aria-label={inWishlist ? 'Remove from wishlist' : 'Add to wishlist'}
      >
        <svg
          className={`w-5 h-5 transition-colors ${inWishlist ? 'text-red-500 fill-red-500' : 'text-gray-400 fill-gray-400'}`}
          viewBox="0 0 24 24"
        >
          <path
      ...
      <div className="flex-1 flex flex-col gap-3 px-1 pb-1">
        ...
        <div className="mt-auto space-y-2">
          ...
          {/* Quick Add to Cart */}
          <button
            className="w-full bg-emerald-500 hover:bg-emerald-600 text-white font-semibold py-2 px-4 rounded-xl transition-all duration-200 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-emerald-400"
