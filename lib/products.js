...
const INDEX_FILE_PATH = path.join(process.cwd(), 'data', 'flexsearch_index.json');
...
export async function loadAndIndexProducts() {
    ...
    console.log("Attempting to load pre-built index...");
    try {
        if (existsSync(INDEX_FILE_PATH)) {
            const indexData = await fsPromises.readFile(INDEX_FILE_PATH, 'utf8');
            const { serializedIndex, loadedProducts } = JSON.parse(indexData);
            ...
            for (const key in serializedIndex) {
                productIndex.import(key, serializedIndex[key]);
            }
            
            products = loadedProducts;

            console.log("Pre-built index loaded successfully.");
            isDataLoaded = true;
            return;
        }
    } catch (e) {
        console.warn("Failed to load pre-built index, building from CSV:", e);
    }
...
async function saveIndex() {
    ...
    await productIndex.export((key, data) => {
    ...
    try {
        await fsPromises.writeFile(INDEX_FILE_PATH, JSON.stringify(dataToSave), 'utf8');
        console.log("FlexSearch index saved successfully.");
    } catch (e) {
        console.error("Failed to save FlexSearch index:", e);
    }
}
