...
const REMOTE_URL = process.env.PRODUCTS_URL || '';
...
async function loadProductsData() {
    if (REMOTE_URL) {
        console.log(`Fetching products from remote URL: ${REMOTE_URL}`);
        const response = await fetch(REMOTE_URL);
        if (!response.ok) {
            throw new Error(`Failed to fetch products from URL: ${response.statusText}`);
        }
        const contentType = response.headers.get('content-type') || '';
        if (contentType.includes('application/json') || REMOTE_URL.endsWith('.json')) {
            const data = await response.json();
            return Array.isArray(data) ? data.map(processProductRow) : [];
        }
        ...
        await new Promise((resolve, reject) => {
            response.body
                .pipe(csv())
                .on('data', row => loaded.push(processProductRow(row)))
                .on('end', () => resolve())
                .on('error', reject);
        });
        return loaded;
    ...
    if (!existsSync(CSV_FILE_PATH)) {
        throw new Error('No product data found. Provide PRODUCTS_URL or products.csv');
    }
